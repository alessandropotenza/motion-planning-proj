#!/usr/bin/env python3
# -----------------------------------------------------------------------------
# Bar plots from eval_ee_goal_franka detailed_log.csv (checkpoint rows):
#   (1) mean iterations to first feasible path (first_path_iteration)
#   (2) mean final path cost at the checkpoint (final_path_cost)
# Franka (7-DOF) counterpart of 2Dexamples/data_analysis2D.py.
# -----------------------------------------------------------------------------
...
PLANNERS: Tuple[str, ...] = ("vanilla", "cdf", "pullandslide")
DISPLAY_LABELS = {
    "vanilla": "Vanilla RRT*",
    "cdf": "CDF RRT*",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Bar charts from Franka detailed_log.csv: iterations to first feasibility "
        "and final path cost (checkpoint rows only)."
    )
    parser.add_argument(
        "--csv", type=Path, required=True,
        help="Path to detailed_log.csv from eval_ee_goal_franka.py",
    )
    parser.add_argument(
        "--checkpoint", "--iteration-budget",
        type=int, dest="checkpoint", metavar="N", default=None,
        help="Iteration budget (must match a checkpoint row). "
        "If omitted, uses the largest checkpoint present in the CSV.",
    )
    parser.add_argument(
        "--list-checkpoints", action="store_true",
        help="Print available iteration_budget values and exit.",
    )
    parser.add_argument(
    ...
    if not rows:
        raise SystemExit(
            f"No checkpoint rows for iteration_budget={checkpoint}. Available: {avail}"
        )
